...
lock = Lock()
temp_csv_klines_dir = os.path.join(
    script_dir, f"binance_scraper_klines_dir_{uuid.uuid4().hex}"
)
temp_csv_premium_index_dir = os.path.join(
    script_dir, f"binance_scraper_premium_index_dir_{uuid.uuid4().hex}"
)
...
def get_metrics_zip_links(coin, time_frame, num_of_days, data_type):
    ...
    if data_type != "futures" or time_frame < 5:
        return None
...
def download_and_extract_zip(args):
    ...
    temp_data_list = process_extracted_files(
        extracted_files, csv_column_names, temp_csv_dir
    )
...
def get_user_input():
    data_type_prompt = [
        inquirer.List(
            "data_type", message="Enter the data type", choices=list_of_market_types
        )
    ]
    coin_prompt = [inquirer.Text("coin", message="Enter coin (e.g. BTCUSDT)")]
    time_frame_prompt = [
        inquirer.List(
            "time_frame", message="Enter the time frame", choices=list_of_time_frames
        )
    ]
    select_date_type_prompt = [
        inquirer.List(
            "date_type",
            message="How much data you want to scrape",
            choices=["date range", "number of days"],
        )
    ]
    ...
    if date_type == "date range":
        ...
        num_of_days_prompt = [
            inquirer.Text(
                "num_of_days",
                message="Enter the number of previous days you want to scrape",
            )
        ]
...
if __name__ == "__main__":
    ...
    if data_type == "futures":
        premium_index_klines_zip_links = get_premium_index_klines_zip_links(
            coin, time_frame, num_of_days, data_type
        )

    if not os.path.exists(temp_csv_klines_dir):
        os.makedirs(temp_csv_klines_dir)

    if data_type == "futures" and metrics_zip_links:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        klines_futures = [
            executor.submit(
                ...
                (i, link, csv_klines_columns, temp_csv_klines_dir),
            )
        ...
        if data_type == "futures" and metrics_zip_links:
            premium_index_klines_futures = [
                executor.submit(
                    download_and_extract_zip,
                    (i, link, premium_index_klines_columns, temp_csv_klines_dir),
                )
        ...
        for future in concurrent.futures.as_completed(all_futures):
            ...
            elif future in premium_index_klines_futures and data_type == "futures":
                temp_data_list_premium_index_klines = future.result()
                if temp_data_list_premium_index_klines is not None:
                    data_list_premium_index_klines.extend(
                        temp_data_list_premium_index_klines
                    )

    if data_type == "spot" and data_list_klines:
        df_klines = pd.concat(data_list_klines).sort_values("open_time")
        df_klines.to_csv(
            os.path.join(
                script_dir, f"{coin.lower()}_{time_frame}_{num_of_days}_klines.csv"
            ),
            index=False,
        )

        is_klines_unique = df_klines.open_time.is_unique
        is_klines_sorted = df_klines.open_time.is_monotonic_increasing
        is_klines_duplicates = df_klines.duplicated(subset=["open_time"]).any()

        if is_klines_unique and is_klines_sorted and not is_klines_duplicates:
            print(
                f"Data has been successfully downloaded and merged to single csv file"
            )
            print(f"Number of rows: {len(df_klines)}")
        else:
            raise Exception("Data is not unique or sorted")

        delete_dir(temp_csv_klines_dir)

    if data_type == "futures" and data_list_klines and data_list_premium_index_klines:
        df_klines = pd.concat(data_list_klines).sort_values("open_time")
        df_premium_index = pd.concat(data_list_premium_index_klines).sort_values(
            "open_time"
        )
        ...
        df_merged.to_csv(
            os.path.join(
                script_dir,
                f"{coin.lower()}_{time_frame}_{num_of_days}_klines_premium_index.csv",
            ),
            index=False,
        )
